...
namespace Managers
{
    ...
    public class WallManager
    {
        ...
        public void Initialize(ResponseObservation observation)
        {
            ...
            int radius = 12;
            for (int x = Math.Max(0, sx - radius); x < Math.Min(width, sx + radius); x++)
            {
                for (int y = Math.Max(0, sy - radius); y < Math.Min(height, sy + radius); y++)
                {
                    int index = x + y * width;
                    if (pathData[index] == 255 && placeData[index] == 0)
                    {
            ...
            for (int i = -1; i <= 1; i++)
            {
                ...
                Point2D pos = FindNearestBuildable(px, py, placeData, width, height);
                if (pos != null)
